...
func InitConfig(fileName string) {
	...
	v.OnConfigChange(func(in fsnotify.Event) {
		...
		if err != nil {
			panic(fmt.Errorf("error changed unmarshal config %s: %v", fileName, err))
		}
	})
	err := v.ReadInConfig()
	if err != nil {
		panic(fmt.Errorf("error reading config:%s: %v", fileName, err))
	}

	// 解析
	err = v.Unmarshal(&Conf)
	if err != nil {
		panic(fmt.Errorf("error unmarshal config %s: %v", fileName, err))
	}
}
